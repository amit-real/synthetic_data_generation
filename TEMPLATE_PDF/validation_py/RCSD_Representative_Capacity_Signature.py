import types
from typing import Literal

from pydantic import BaseModel, Field
from pydantic import BaseModel, ValidationError, model_validator


class BaseDoc(BaseModel):
    @model_validator(mode="before")
    @classmethod
    def update_keys(self, json_data: dict) -> dict:
        new_dict = {}
        for key, value in json_data.items():
            if key == "text_fields":
                for name in json_data["text_fields"]:
                    if (
                        len(json_data["text_fields"][name]) == 0
                        or json_data["text_fields"][name] is None
                    ):  # removing empty strings
                        continue
                    normalized_key = (
                        name.replace("<", "").replace(">", "").replace("-", "_").lower()
                    )
                    new_dict[normalized_key] = json_data["text_fields"][name]
            elif key == "checkboxes":
                for name in json_data["checkboxes"]:
                    normalized_key = (
                        name.replace("<", "").replace(">", "").replace("-", "_").lower()
                    )
                    new_dict[normalized_key] = json_data["checkboxes"][name]["state"]
            elif key == "signatures":
                for name in json_data["signatures"]:
                    normalized_key = (
                        "sign_" + name.replace("<", "").replace(">", "").replace("-", "_").lower()
                    )
                    new_dict[normalized_key] = True
            else:
                normalized_key = key.replace("<", "").replace(">", "").replace("-", "_").lower()
                new_dict[normalized_key] = value
        return new_dict
    

class RCSD_Representative_Capacity_Signature_Page1(BaseDoc):
    agreement_1: str = Field(None, description="agreement_1", error_message="agreement_1 is missing")
    agreement_2: str = Field(None, description="agreement_2", error_message="agreement_2 is missing",)
    date_1: str = Field(description="date_1", error_message="date_1 is missing")
    address_1: str = Field(None, description="address_1", error_message="address_1 is missing")

    name_1: str = Field(None, description="name_1", error_message="name_1 is missing")
    name_2: str = Field(None, description="name_2", error_message="name_2 is missing",)
    date_2: str = Field(description="date_2", error_message="date_2 is missing")
    entity: str = Field(None, description="entity", error_message="entity is missing")
    date_3: str = Field(None, description="date_3", error_message="date_3 is missing",)
    case_num: str = Field(description="case_num", error_message="case_num is missing")
    seller_1_name: str = Field(None, description="seller_1_name", error_message="seller_1_name is missing")
    seller_2_name: str = Field(None, description="seller_2_name", error_message="seller_2_name is missing")
    
    seller_1_representative: str = Field(None, description="seller_1_representative", error_message="seller_1_representative is missing")
    seller_2_representative: str = Field(None, description="seller_2_representative", error_message="seller_2_representative is missing")
    date_4: str = Field(None, description="date_4", error_message="date_4 is missing")
    date_5: str = Field(description="date_5", error_message="date_5 is missing")
    date_6: str = Field(None, description="date_6", error_message="date_6 is missing")
    date_7: str = Field(description="date_7", error_message="date_7 is missing")
    buyer_1_name: str = Field(None, description="buyer_1_name", error_message="buyer_1_name is missing")
    buyer_2_name: str = Field(None, description="buyer_2_name", error_message="buyer_2_name is missing")
    trust_name_1: str = Field(None, description="trust_name_1", error_message="trust_name_1 is missing")
    trust_name_2: str = Field(description="trust_name_2", error_message="trust_name_2 is missing")
    trust_name_3: str = Field(None, description="trust_name_3", error_message="trust_name_3 is missing")
    trust_name_4: str = Field(description="trust_name_4", error_message="trust_name_4 is missing")
    estate: str = Field(description="estate", error_message="estate is missing")
      

class DocumentModel(BaseModel):
    page_1: RCSD_Representative_Capacity_Signature_Page1


PAGE_SCHEMA_MAPPING = {1: RCSD_Representative_Capacity_Signature_Page1}
